...
describe Zoom::Actions::SipAudio do
  ...
  describe "#sip_trunk_numbers_delete action" do
    context "with a valid response" do
      ...
      it "returns the http status code as a number" do
        expect(zc.sip_trunk_numbers_delete(args)).to eql(204)
      end
    end
    ...
    context "with a 4xx response" do
      ...
      it "raises Zoom::Error exception" do
        expect { zc.sip_trunk_numbers_delete(args) }.to raise_error(Zoom::Error)
      end
    end
  end
end
