...
RSpec.describe Zoom::Params do
  ...
  describe "#require_one_of" do
    ...
    it "does raise an error when there is a missing required keys out of many" do
      expect { params.require_one_of(:bar, :fooey, :fooa) }.to raise_error(Zoom::ParameterMissing, "You are missing atleast one of [:bar, :fooey, :fooa]")
    end
  ...
  describe "#permit_value" do
    ...
    it "does raise an error when required keys are missing" do
      expect { params.permit_value(:baz, values) }.to raise_error(Zoom::ParameterValueNotPermitted, "baz: bang")
    end
  end
